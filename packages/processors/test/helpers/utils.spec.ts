...
describe("utils", () => {
    describe("getDateFromTimestamp", () => {
        it("converts a valid timestamp to a Date object", () => {
        ...
        it("handles the minimum valid timestamp (0)", () => {
            const timestamp = 0n;
            const result = getDateFromTimestamp(timestamp);
            expect(result).toBeInstanceOf(Date);
        ...
        it("handles the maximum valid timestamp", () => {
            const maxTimestamp = 18446744073709551615n - 1n;
            const result = getDateFromTimestamp(maxTimestamp);
            expect(result).toBeInstanceOf(Date);
        });

        it("returns null for timestamps equal to or greater than UINT64_MAX", () => {
